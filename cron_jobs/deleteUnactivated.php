...
if($numrows > 0){
//we can get the row by the name by using mysqloi_assoc
while($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
  $id = $row['id'];
  $username = $row['username'];
  $userFolder = "../user/$username";
  if(is_dir($userFolder)) {
          rmdir($userFolder);//bash to remove their directory ofcourse the directory is empty
      }
  //each time delete one
